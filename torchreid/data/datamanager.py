...
class ImageHybridDatamanager(DataManager):
    r"""Image data manager with two kinds of sampling methods, namely hybrid data manager.
        All nontent is the same as ImageDataManager, except an extra trainloader with different sampling sampler.

    Args:
        train_sampler (str, optional): sampler. Example: 'RandomSampler TripletSampler', which will be seperated by ' ' into two samplers
    ...
    def __init__(self, root='', sources=None, targets=None, height=256, width=128, transforms='random_flip', 
                 ...
                 batch_size_train=32, batch_size_test=32, workers=4, num_instances=4, train_sampler='',
                 cuhk03_labeled=False, cuhk03_classic_split=False, market1501_500k=False):
        super(ImageHybridDatamanager, self).__init__(sources=sources, targets=targets, height=height, width=width,
                                               transforms=transforms, norm_mean=norm_mean, norm_std=norm_std,
                                               use_gpu=use_gpu)
        ...
        train_sampler_1 = train_sampler.split(' ')[0]
        train_sampler1 = build_train_sampler(
            trainset.train, train_sampler_1,
            batch_size=batch_size_train,
            num_instances=num_instances
        )

        self.trainloader1 = torch.utils.data.DataLoader(
            trainset,
        ...
        train_sampler_2 = train_sampler.split(' ')[1]
        train_sampler2 = build_train_sampler(
            trainset.train, train_sampler_2,
            batch_size=batch_size_train,
            num_instances=num_instances
        )

        self.trainloader2 = torch.utils.data.DataLoader(
            trainset,
    ...
    def return_dataloaders(self):
        """Returns trainloader and testloader."""
        return self.trainloader1, self.trainloader2, self.testloader
