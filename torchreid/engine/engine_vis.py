...
class Engine(object):
    ...
    def test(self,
        ...
        if multi_eval and len(targets) > 1:
            ...
            rank1 = self._evaluate_multi(epoch,
            ...
            for batch_idx, data in enumerate(queryloader):
                ...
                for j in range(outputs.size(0)):
                    ...
                    img_np = img_np.transpose(
                        (1, 2, 0))  # (c, h, w) -> (h, w, c)
                    img_np_save = img_np.copy()
                    cv2.imwrite('ori_%d.png' % j, cv2.cvtColor(img_np_save, cv2.COLOR_RGB2BGR))
                    # activation map
                    am = outputs[j, ...].numpy()
                    ...
                    am_save = am.copy()
                    cv2.imwrite('h_m_%d.png' % j, am_save)
                    ...
                    #data1 = pd.DataFrame(overlapped[:,:,0])
                    #data2 = pd.DataFrame(overlapped[:,:,1])
                    #data3 = pd.DataFrame(overlapped[:,:,2])
                    ...
                    array=np.resize(overlapped,[overlapped.shape[0],overlapped.shape[1]*overlapped.shape[2]])
                    np.savetxt(imname +"example.csv",array,delimiter=',')
                    #writer.close() 
                    #np.savetxt('new.csv', overlapped, delimiter = ',')
                    #file=open('test.txt','a+')
                    #for j in range(overlapped.shape[0]):            #遍历行
                    #    for i in range(overlapped.shape[1]):        #遍历列
                    #        file.write(' '+str(overlapped[j, i]))          #写入像素点值
                    #        if i==overlapped.shape[1]-1:               #判断是否位行最后一位
                    #            file.write("\n")                        #换行
